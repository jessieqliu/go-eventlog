...
func TestParseEventLogWindows(t *testing.T) {
	testParseEventLog(t, "../testdata/legacydata/windows_gcp_shielded_vm.json")
}

func TestParseEventLogLinux(t *testing.T) {
	testParseEventLog(t, "../testdata/legacydata/linux_tpm12.json")
}
...
func TestParseCryptoAgileEventLog(t *testing.T) {
	data, err := os.ReadFile("../testdata/legacydata/crypto_agile_eventlog")
	if err != nil {
		t.Fatalf("reading test data: %v", err)
	}
...
func TestEventLogLinux(t *testing.T) {
	testEventLog(t, "../testdata/legacydata/linux_tpm12.json")
}

func TestEventLog(t *testing.T) {
	testEventLog(t, "../testdata/legacydata/windows_gcp_shielded_vm.json")
}
...
func convertToMRs(pcrs []register.PCR) []register.MR {
	mrs := make([]register.MR, len(pcrs))
	for i, v := range pcrs {
		mrs[i] = v
	}
	return mrs
...
func TestParseShortNoAction(t *testing.T) {
	...
	data, err := os.ReadFile("../testdata/legacydata/short_no_action_eventlog")
	if err != nil {
		t.Fatalf("reading test data: %v", err)
	}
	if _, err := ParseEventLog(data, ParseOpts{}); err != nil {
...
func TestEBSVerifyWorkaround(t *testing.T) {
	pcr5 := []register.MR{
		register.PCR{
			...
			DigestAlg: register.HashSHA1.CryptoHash(),
		},
		register.PCR{
			Index: 5,
			Digest: []byte{
			...
			DigestAlg: register.HashSHA256.CryptoHash(),
		},
	}

	elr, err := os.ReadFile("../testdata/legacydata/ebs_event_missing_eventlog")
	if err != nil {
		t.Fatal(err)
	}
...
func TestAppendEvents(t *testing.T) {
	base, err := os.ReadFile("../testdata/legacydata/ubuntu_2104_shielded_vm_no_secure_boot_eventlog")
	if err != nil {
		t.Fatalf("reading test data: %v", err)
	}
